...
for option in soup.find('select', id='term_dropdown').findAll('option'):
	if option.getText() == sys.argv[1]:
		termStr = option['value']
		break
if termStr == "":
	print "Couldn't find that term!"
	sys.exit(1)

br.select_form(name='searchForm')
br['binds[:term]'] = [termStr]
